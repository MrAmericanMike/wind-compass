...
export class WindCompass {
	...
	private LONG_FORMAT: CompassPoints = {
		"N": "North",
	...
	private SHORT_FORMAT: CompassPoints = {
		"N": "N",
		"NNE": "NNE",
		"NE": "NE",
		"ENE": "ENE",
	 ...
	 */
	getWindFromDirection(degree: number, longFormat: boolean = true): string | never {
	 ...
	 */
	private validateCompassPoints(compassPoints: CompassPoints, formatType: FormatType) {
		if (typeof compassPoints !== "object" || compassPoints === null) {
			throw new Error(`${formatType} must be an object`);
		}

		const VALID_KEYS = Object.keys(this.LONG_FORMAT) as (keyof CompassPoints)[];
		const CHECK_KEYS = Object.keys(compassPoints) as (keyof CompassPoints)[];
